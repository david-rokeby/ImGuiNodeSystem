...
/*static struct {
  ...
  if (Window->SkipItems)
    ...
    int changed = 0;
  // header and spacing
/*    ImGui::PushItemWidth(AREA_WIDTH + safe.x * 3);
    ImGui::PushID("A");
    changed |= SliderFloat(label, &P[0], 0, 1, "%.3f", 1.0f);
    ImGui::PopID();
    ImGui::PushID("B");
    changed |= SliderFloat(label, &P[1], 0, 1, "%.3f", 1.0f);
    ImGui::PopID();
    ImGui::PushID("C");
    changed |= SliderFloat(label, &P[2], 0, 1, "%.3f", 1.0f);
    ImGui::PopID();
    ImGui::PushID("D");
    changed |= SliderFloat(label, &P[3], 0, 1, "%.3f", 1.0f);
    ImGui::PopID();
    ImGui::PopItemWidth();*/
...
//  Dummy(ImVec2(dim, dim));
  ImVec2 Canvas(dim, dim);

  ImRect bb(Window->DC.CursorPos + safe, Window->DC.CursorPos + Canvas + safe + safe);
  ItemSize(bb);
  if (!ItemAdd(bb, NULL))
    return changed;
  ...
  hovered |=
    ...
    ImGui::PushID(id);
    ImVec2 halfSafe = {safe.x / 2, safe.y / 2};
    ImRect bbInner( bb.Min + halfSafe, bb.Max - halfSafe);
    RenderFrame(bbInner.Min, bbInner.Max, GetColorU32(ImGuiCol_WindowBg), true,
              Style.FrameRounding);

  // background grid
  for (int i = 0; i <= Canvas.x; i += (Canvas.x / 4)) {
    DrawList->AddLine(ImVec2(bbInner.Min.x + i, bbInner.Min.y),
                      ImVec2(bbInner.Min.x + i, bbInner.Max.y),
                      GetColorU32(ImGuiCol_FrameBg));
  }
  for (int i = 0; i <= Canvas.y; i += (Canvas.y / 4)) {
    DrawList->AddLine(ImVec2(bbInner.Min.x, bbInner.Min.y + i),
                      ImVec2(bbInner.Max.x, bbInner.Min.y + i),
                      GetColorU32(ImGuiCol_FrameBg));
  }
  ...
  // control points: 2 lines and 2 circles
  {
    ...
    for (int i = 0; i < 2; ++i)
    {
      pos[i] = ImVec2(P[i * 2 + 0], 1 - P[i * 2 + 1]) * (bbInner.Max - bbInner.Min) + bbInner.Min;
  ...
  // draw curve
  {
    ...
    ImColor color(GetStyle().Colors[ImGuiCol_TextSelectedBg]);
    for (int i = 0; i < SMOOTHNESS; ++i) {
      ImVec2 p = {results[i + 0].x, 1 - results[i + 0].y};
      ImVec2 q = {results[i + 1].x, 1 - results[i + 1].y};
      ImVec2 r(p.x * (bbInner.Max.x - bbInner.Min.x) + bbInner.Min.x,
               p.y * (bbInner.Max.y - bbInner.Min.y) + bbInner.Min.y);
      ImVec2 s(q.x * (bbInner.Max.x - bbInner.Min.x) + bbInner.Min.x,
               q.y * (bbInner.Max.y - bbInner.Min.y) + bbInner.Min.y);
  ...
  ImVec4 pink(0.5f, 0.5f, 0.5f, luma), cyan(0.5f, 0.5f, 0.5f, luma);
  ImVec2 p1 = ImVec2(P[0], 1 - P[1]) * (bbInner.Max - bbInner.Min) + bbInner.Min;
  ImVec2 p2 = ImVec2(P[2], 1 - P[3]) * (bbInner.Max - bbInner.Min) + bbInner.Min;
  DrawList->AddLine(ImVec2(bbInner.Min.x, bbInner.Max.y), p1, ImColor(pink), LINE_WIDTH);
  DrawList->AddLine(ImVec2(bbInner.Max.x, bbInner.Min.y), p2, ImColor(pink), LINE_WIDTH);
  ...
  DrawList->AddCircleFilled(p2, GRAB_RADIUS - GRAB_BORDER, ImColor(cyan));
    ...
    ImGui::PopID();
    ImGui::Dummy(ImVec2(Canvas.x + safe.x * 3, safe.y));
